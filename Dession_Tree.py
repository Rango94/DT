...
class DessionTree:
    def __init__(self,Label_type='con',cate='val',max_deep=6):
        self.max_deep=max_deep  #最大深度
        self.Label_type=Label_type    #数据类型是连续还是离散
        self.cate=cate  #决策树类型，均方误差 还是信息增益 还是信息增益率
    ...
    def fit(self,X,Y):
        ...
        self.split_the_X(X)
        print('建立完毕')
        if self.Label_type== 'con' and self.cate!= 'val':
            print('将连续标签离散化')
    ...
    def Split(self, tree, X, Y,route):
        if tree.deep <= self.max_deep and Y.var()!=0:
            best_idx = 0
            best_point = 0
            best_value = 0
            x_left = []
    ...
    def select_the_point(self, x, y, IDX, point):
        ...
        if self.cate=='id3':
            E = self.Entropy(y)
            return E-(len(left_y)/len(y))*self.Entropy(left_y)+(len(right_y)/len(y))*self.Entropy(right_y),\
                   np.array(left_x),\
                   np.array(left_y),\
                   np.array(right_x),\
                   np.array(right_y)
...
if __name__=='__main__':
    ...
    x_train=X[500:]
    y_train=Y[500:]
    print('训练样本数量%d'%len(y_train))
    print('测试样本数量%d' % len(y_test))
    cate='id3'
    DT=DessionTree('con', max_deep=6, cate=cate)
    DT.fit(x_train, y_train)
    DT.save_model('basemodel.' + cate)

    DT=DessionTree()
    DT.load_model('basemodel.'+cate)
